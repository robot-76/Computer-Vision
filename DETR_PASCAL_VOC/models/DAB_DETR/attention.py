...
def multi_head_attention_forward(query: Tensor,
    ...
    esp = 1e-5  # or any other value you prefer
    ...
    ln_q = nn.LayerNorm(q.size(-1), eps=esp).to(device)
    ln_k = nn.LayerNorm(k.size(-1), eps=esp).to(device)
    ln_v = nn.LayerNorm(v.size(-1), eps=esp).to(device)
    ...
    # Scale to range [-1, 1]
    q = (q - q.min()) / (q.max() - q.min()) * 2 - 1
    k = (k - k.min()) / (k.max() - k.min()) * 2 - 1
    v = (v - v.min()) / (v.max() - v.min()) * 2 - 1"""

    # Alternatively, if you want to ensure that the scaling is done per sample or per batch
    # Normalize each sample within the batch
    q = (q - q.min(dim=-1, keepdim=True)[0]) / (q.max(dim=-1, keepdim=True)[0] - q.min(dim=-1, keepdim=True)[0]) * 2 - 1
    k = (k - k.min(dim=-1, keepdim=True)[0]) / (k.max(dim=-1, keepdim=True)[0] - k.min(dim=-1, keepdim=True)[0]) * 2 - 1
    v = (v - v.min(dim=-1, keepdim=True)[0]) / (v.max(dim=-1, keepdim=True)[0] - v.min(dim=-1, keepdim=True)[0]) * 2 - 1
