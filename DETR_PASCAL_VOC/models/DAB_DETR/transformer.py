...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            query_sine_embed2 = gen_sineembed_for_position(obj_center, self.d_model)  
            query_pos2 = self.ref_point_head(query_sine_embed2)  
            ...
            query_sine_embed2 = query_sine_embed2[...,:self.d_model] * pos_transformation
            ...
            if self.modulate_hw_attn:
                ...
                query_sine_embed2[..., self.d_model // 2:] *= (refHW_cond[..., 0] / obj_center[..., 2]).unsqueeze(-1)
                query_sine_embed2[..., :self.d_model // 2] *= (refHW_cond[..., 1] / obj_center[..., 3]).unsqueeze(-1)
            ...
            output = layer(output, memory, tgt_mask=tgt_mask,
                           ...
                           query_pos2=query_pos2, query_sine_embed2=query_sine_embed2,
                           is_first=(layer_id == 0))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            tgt = self.self_attn(q, k, value=v, attn_mask=tgt_mask,
            ...
            # Apply projections for the second self-attention block
            ...
            tgt, attn_weights_2 = self.self_attn(q, k, value=v, attn_mask=tgt_mask,
                                    key_padding_mask=tgt_key_padding_mask)
            # ========== End of Second Self-Attention =============
        ...
        tgt = self.cross_attn(query=q,
                                   key=k,
                                   value=v, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]               
        # ========== End of Cross-Attention =============
        
        #tgt = tgt + self.dropout4(tgt)
        tgt = self.norm4(tgt)
        
        # ========== Begin of Cross-Attention 2 =============
        q_content = self.ca_qcontent_proj(tgt)
        k_content = self.ca_kcontent_proj(memory)
        v = self.ca_v_proj(memory)
        ...
        # the object query (the positional embedding) into the original query (key) in DETR.
        if is_first or self.keep_query_pos:
            q_pos = self.ca_qpos_proj(query_pos2)
        ...
        q = q.view(num_queries, bs, self.nhead, n_model//self.nhead)
        query_sine_embed2 = self.ca_qpos_sine_proj(query_sine_embed2)
        query_sine_embed2 = query_sine_embed2.view(num_queries, bs, self.nhead, n_model//self.nhead)
        q = torch.cat([q, query_sine_embed], dim=3).view(num_queries, bs, n_model * 2)
        k = k.view(hw, bs, self.nhead, n_model//self.nhead)
        k_pos = k_pos.view(hw, bs, self.nhead, n_model//self.nhead)
        k = torch.cat([k, k_pos], dim=3).view(hw, bs, n_model * 2)
        ...
        tgt = self.cross_attn(query=q,
                                   ...
                                   value=v, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]               
        # ========== End of Cross-Attention 2 =============
        #print("Does c2tgt_cond contain NaN values?", torch.isnan(tgt3).any())

        tgt = tgt + self.dropout5(tgt)
        tgt = self.norm5(tgt)
        tgt = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
        tgt = tgt + self.dropout6(tgt)
        tgt = self.norm6(tgt)
        return tgt
